...
# ================= 标题 / 作者 / 单位 =================
para("遗传预测的血清促甲状腺激素水平与甲状腺癌风险的孟德尔随机化"
     "重复验证与扩展研究：附候选基因评分与肿瘤微环境的探索性评估", size=16, bold=True,
...
para("【背景】血清促甲状腺激素（TSH）水平与甲状腺癌发病风险的因果关系存在争议，"
     "且既往研究基于不同的工具变量集与结局定义结论不一。本研究定位为对既往工具变量集的重复验证与扩展："
     "以两样本孟德尔随机化（MR）为核心评估 TSH-甲状腺癌因果关联，报告完整敏感性分析；"
     "候选基因评分（含样本外负结果检验）、免疫浸润与药物敏感性计算预测作为探索性补充。", first_indent=22)
para("【方法】基于 Zhou 等（2020）的 TSH 全基因组关联研究补充数据（99 个全基因组显著位点，"
     ...
     "在 TCGA-THY 队列（409 例）中构建探索性候选基因评分（明确报告样本外负结果），"
...
para("【结果】两样本 MR 显示遗传预测 TSH 升高与甲状腺癌风险降低存在因果关联"
     ...
     "鉴于异质性与离群检验均显著，将 MR-PRESSO 离群校正估计与 IVW 并列为主分析："
     "剔除 4 个离群 SNP 后（65 个 SNP）随机效应 IVW 估计 OR = 0.579（95% CI 0.452–0.741，"
     "P = 1.44×10⁻⁵），与未校正 IVW 方向一致、效应量略向零收缩；MR-RAPS 估计方向亦一致"
     "（OR = 0.476，P = 4.2×10⁻¹⁵）。两个复制结局方向一致"
     "（OR = 0.362，P = 6.10×10⁻⁶；OR = 0.559，P = 0.0025）。MVMR 在校正 BMI 与睾酮后 TSH 效应方向仍一致（54 SNP，"
     "OR = 0.525，95% CI 0.317–0.869，P = 0.0123），但 BMI 与睾酮为弱工具"
     "（条件 F 分别为 5.66、3.38）且条件异质性显著，TSH 独立效应无法确认，该结果仅具探索性。"
     ...
     "5×5 折重复交叉验证 C-index 均值 0.334（低于随机水平 0.5，样本外无预测能力）；GSE33630 中评分呈 "
     "ATC > PTC > Normal 病理类型梯度（Kruskal-Wallis P = 1.75×10⁻¹²，非生存验证）。"
     ...
     first_indent=22)
para("【结论】作为重复验证与扩展研究，遗传证据再次支持 TSH 升高与甲状腺癌发病风险降低相关，"
     "但工具变量间异质性高、MVMR 受弱工具限制且无共定位证据，因果证据强度有限；"
     "GNG7/NFIA 候选基因评分经重复交叉验证显示样本外无预测能力（C-index 0.334），为负结果；"
     "免疫浸润与药物敏感性均为计算预测、假设生成性质。临床价值需独立队列与实验验证。", first_indent=22)
para("【关键词】促甲状腺激素；甲状腺癌；孟德尔随机化；重复验证；样本外验证",
...
para("孟德尔随机化（MR）利用遗传变异作为工具变量，可有效削弱混杂与反向因果的影响。"
     ...
     first_indent=22)
para("本研究定位为重复验证与扩展分析：使用 Zhou 等（2020）已发表的 TSH 工具变量集，"
     "在新的主结局与两个独立复制结局中系统评估 TSH-甲状腺癌因果关联，"
     "报告 MR-PRESSO 校正估计、MR-RAPS、Steiger 与完整异质性/多效性检验"
     "（全部数值来自脚本 A 与脚本 L 的真实 Console 输出）。在此基础上行探索性延伸："
     "（1）校正 BMI 与睾酮的 MVMR（如实标注弱工具限制）；"
     "（2）将工具 SNP 映射至候选基因，在 TCGA-THY 构建候选基因评分，"
     "以 Bootstrap 变量选择频率与 5×5 折重复交叉验证明确报告其样本外无预测能力（负结果），"
     "GEO 队列仅行病理类型梯度（非生存）验证；"
     "（3）免疫浸润与 GDSC2 药物敏感性计算预测（报告交叉验证性能与置换检验）。"
...
para("补充数据提供的 99 个位点均已满足 P < 5×10⁻⁸；在此基础上以连锁不平衡 r² < 0.001"
     ...
     "以 harmonise_data（action = 2）进行等位基因协调：回文 SNP 按等位基因频率判定，"
     "无法判定的回文与等位基因不匹配 SNP 予以剔除，主结局协调后余 69 个有效 SNP。"
     "暴露 GWAS（HUNT/MGI/ThyroidOmics，不含 UK Biobank）与结局 GWAS（欧洲人群）"
     "队列构成不同，但均为欧洲血统人群，无法完全排除样本重叠且未量化（见 4.7）。"
...
heading("2.4  候选基因评分的探索性构建（负结果检验）", 2)
para("将 MR 工具 SNP 映射至候选基因（55 个 SNP → 60 个候选基因），"
     ...
     "鉴于多因素 Cox 中两个基因均未达传统显著阈值（GNG7 P = 0.072、NFIA P = 0.241），"
     "本研究将样本外预测能力检验（5×5 折重复交叉验证 C-index）预设为负结果判定的关键指标。",
     first_indent=22)

heading("2.5  GEO 病理类型梯度验证（非生存验证）", 2)
...
para("以 GDSC2 表达矩阵（17419 基因 × 805 细胞系）与 IC50 矩阵（805 细胞系 × 198 药物，"
     ...
     "为评估预测可靠性：（1）对 12 个代表性药物（5-Fluorouracil、Paclitaxel、Docetaxel、"
     "Cisplatin、Camptothecin、Vinblastine、Sorafenib、Nilotinib、GSK591、VE821、Daporinad、"
     "Selumetinib，覆盖细胞毒化疗与靶向/表观遗传类作用机制）在 GDSC2 训练集行 5 折交叉验证"
...
para("两样本 MR 主分析（69 个有效 SNP）中，IVW 随机效应模型显示遗传预测 TSH 升高与"
     ...
     "P = 0.091）方向一致但未达显著。鉴于 Cochran Q 与 MR-PRESSO 全局检验均提示"
     "显著异质性/离群，本研究将 MR-PRESSO 离群校正估计与 IVW 并列作为主分析："
     "按 MR-PRESSO 离群检验识别的 4 个离群 SNP（rs10186921、rs116909374、rs2993047、"
     "rs925488；失真检验显示剔除前后因果估计无显著变化，失真系数 −31.5%，P = 0.119）"
     "剔除后，随机效应 IVW 估计（65 个 SNP）OR = 0.579，95% CI 0.452–0.741，"
     "P = 1.44×10⁻⁵，与未校正 IVW 方向一致、效应量略向零收缩，提示离群 SNP 未驱动主结论。"
...
add_figure("fig13_mr_funnel.png", 13.0,
           "图 3  两样本 MR 漏斗图（IVW 与 MR-Egger 估计）")
add_figure("fig14_mr_leaveoneout.png", 13.0,
           "图 4  留一法敏感性分析图（剔除单个 SNP 后的 IVW 估计）")

heading("3.3  TSH 与甲状腺癌风险：多变量孟德尔随机化", 2)
para("以 TSH 70 个工具 SNP 为骨架、校正 BMI（ieu-b-40）与总睾酮（ebi-a-GCST90014013）后，"
     "三暴露 MVMR-IVW（54 个三暴露与结局均有效的 SNP）显示校正 BMI 与总睾酮后"
     "TSH 对甲状腺癌的效应方向仍一致（b = −0.645，SE = 0.258，OR = 0.525，"
     "95% CI 0.317–0.869，P = 0.0123，t 检验），BMI（b = −0.525，SE = 2.945，"
     "OR = 0.591，P = 0.859）与睾酮（b = 2.394，SE = 4.699，OR = 10.96，P = 0.613）"
     "均未显示独立效应。条件 F 统计量显示 TSH 工具强度合格"
     ...
     "以 Q 统计量最小化方法（qhet_mvmr，Bootstrap 1000 次，CI = TRUE）估计：TSH b = −0.628"
     "（95% CI −1.203 至 −0.193），BMI b = −0.746、睾酮 b = 6.399"
     "（qhet_mvmr 为区间最小化启发式估计，其 Bootstrap 仅输出点估计与 CI，不输出 SE/P）。"
     "综上，TSH 效应在校正 BMI 与睾酮后方向仍一致，但 BMI/睾酮弱工具与条件异质性"
...
para("基于 MR 阳性结果，将 55 个工具 SNP 映射至 60 个候选基因，在 TCGA-THY 队列"
     ...
     "内部区分度指标（时间依赖 ROC 1/3/5 年 AUC = 0.657/0.612/0.593、"
     "3 年校准 MAE = 0.037、DCA 正净获益阈值 0.01/0.02/0.04–0.08）均源于训练队列、"
     "未经外部生存验证，完整图表见补充材料 4（补充图 1–3）。"
     "变量选择稳定性：Bootstrap 200 次中 GNG7 保留频率 151/200（75.5%）、"
     "NFIA 保留频率 113/200（56.5%）（均以负向系数进入，与评分方向一致）；"
     "5×5 折重复交叉验证 C-index 均值 0.334（范围 0.039-0.826），低于随机水平 0.5，"
     "明确提示该评分无样本外预测能力（负结果）；内部 C-index 0.666 很可能源于过拟合，"
     "该评分不应视为预后模型。"
...
add_figure("fig2_km.png", 13.0,
           "图 5  高、低风险组无复发生存 Kaplan-Meier 曲线（Log-rank P = 0.0412）")
...
add_figure("fig6_geo.png", 13.0,
           "图 6  GSE33630 中风险评分病理类型梯度（Kruskal-Wallis P = 1.75e-12）")

heading("3.6  免疫浸润分析（计算推断，详见补充材料 4）", 2)
para("ssGSEA 显示 28 种免疫细胞的相对富集评分中 25 种（89.3%）在高、低风险组间差异显著"
     "（FDR < 0.05），高风险组 Th1、滤泡辅助 T 细胞、Th2 及活化 CD8 T 细胞等富集，"
     "免疫检查点标志物中 PD-L1（FDR = 3.3×10⁻¹⁴）、PD-L2（FDR = 2.8×10⁻⁹）、"
     "SIGLEC15、CD80、CTLA-4、TIGIT、CD86、TIM-3、TNFRSF9 等 9 个 FDR < 0.05。需注意："
     "ssGSEA 为转录组相对富集推断，不等同于绝对浸润，未校正肿瘤纯度/亚型/驱动突变，"
     "高、低风险组基于同一数据定义存在数据驱动偏倚，结果均为计算推断、假设生成性质；"
     "完整热图与箱线图见补充材料 4（补充图 4–5）。", first_indent=22)

heading("3.7  药物敏感性分析（计算推断，详见补充材料 4）", 2)
para("基于 GDSC2 计算预测 409 例患者对 198 种药物的 IC50，176 种（88.9%）组间差异显著"
     "（FDR < 0.05），高风险组对 158 种预测更耐药、仅 18 种更敏感，整体呈计算预测的耐药特征；"
     "代表药物包括 5-氟尿嘧啶（FDR = 1.6×10⁻¹¹）、紫杉醇（FDR = 9.5×10⁻¹²）、GSK591"
     "（FDR = 8.8×10⁻¹⁶）、VE821（FDR = 3.3×10⁻¹⁵）等耐药，仅 Selumetinib"
     "（FDR = 5.3×10⁻¹⁴）、KU-55933（FDR = 0.0043）等少数预测敏感。可靠性评估："
     "12 个代表药物（覆盖细胞毒与靶向机制）5 折交叉验证 R² 中位数 0.350"
     "（范围 0.080–0.430）、RMSE 中位数 1.478；组标签置换（B = 500 次）显示实际显著药数"
     "176 远超随机期望（零分布中位数 0.62、最大值 82，P < 0.002）。需注意：药物间 IC50"
     "高度相关、独立 FDR 可能高估显著药物数；结果均为基于细胞系模型的计算推断、假设生成性质，"
     "未经实验验证；完整火山图、箱线图与逐药数值见补充材料 4（补充图 6–7、补充表）。",
     first_indent=22)
...
heading("4.1  主要发现", 2)
para("本研究为 TSH-甲状腺癌因果关联的重复验证与扩展分析，以两样本孟德尔随机化为核心"
     "评估遗传预测 TSH 水平与甲状腺癌风险的因果关联，并在此基础上行探索性延伸。"
     ...
     "95% CI 0.317–0.869，P = 0.0123，但受弱工具限制）；其二，由 GNG7 与 NFIA 构建的候选基因评分在训练队列中 C-index = 0.666，"
     "但 5×5 折样本外 C-index 均值 0.334（低于随机水平 0.5），为负结果——"
     "该评分无样本外预测能力；GEO 独立数据集仅支持病理类型梯度（非生存验证）；"
...
para("就 MVMR 而言，既往性激素-甲状腺癌 MR 研究总体未发现强因果关联"
     ...
     "BMI（P = 0.859）与睾酮（P = 0.613）未显示独立效应。需诚实说明：条件 F 统计量显示"
     "BMI（F = 5.66）与睾酮（F = 3.38）的条件工具强度不足（小于 10），且 MVMR 异质性 "
     "Q 统计量显著（Q = 244.76，df = 50，P = 1.81×10⁻²⁷），提示弱工具与水平多效性"
     "可能影响 MVMR 估计；以 Q 统计量最小化方法复核 TSH 方向一致（b = −0.628，95% CI −1.203 至 −0.193），"
     "但 BMI（95% CI −9.059 至 3.277）与睾酮（95% CI −6.003 至 19.826）的效应估计"
     "置信区间极宽，应视为不可靠。因此但 BMI/睾酮的效应估计应视为不可靠。因此，本研究证据不足以断言 TSH 效应"
...
heading("4.3  候选基因评分的负结果解读与机制假设", 2)
...
para("本研究的 MR 结果与既往观察性流行病学证据方向一致，并在两个独立复制结局及"
     ...
     "（均为计算推断，须经实验验证）。需要强调，当前评分为样本外负结果（重复交叉验证 C-index 0.334 低于随机水平），"
     "其训练队列 C-index 0.666 的 95% 置信区间下限（0.559）接近随机水平，且未经独立生存队列验证，"
     "尚不足以支持任何临床决策。", first_indent=22)
...
para("本研究存在以下局限性：（1）候选基因评分尚未在任何独立队列完成生存终点验证"
     ...
     "TSH 直接效应方向虽一致但其精确性受上述因素制约，故本研究不足以断言 TSH 效应独立于"
     ...
     "而未执行；共定位（colocalization）需要暴露 GWAS 完整区域的汇总统计量，"
     "而 Zhou 等（2020）公开数据仅含筛选后的工具位点，无法进行标准共定位分析，"
     "eQTL-MR 亦未执行，故工具 SNP 与候选基因的功能关联仅停留在基因组定位层面；"
...
# ================= 5 结论 =================
heading("5  结论", 1)
para("本研究作为 TSH-甲状腺癌因果关联的重复验证与扩展分析，再次支持遗传预测 TSH 升高与"
     "甲状腺癌发病风险降低相关（主分析 IVW OR = 0.487，PRESSO 校正 OR = 0.579，两个复制结局方向一致），"
     "但工具变量间异质性高（I² = 75.7%）、MVMR 受弱工具限制且共定位证据缺失，因果证据强度有限。"
     "候选基因评分经重复交叉验证显示样本外无预测能力（C-index 0.334，低于随机水平 0.5），"
     "为负结果，不应视为预后模型。免疫浸润与药物敏感性均为计算预测、假设生成性质。"
     "本研究属探索性、假设生成研究，其临床与生物学意义需独立生存队列、实验验证与机制学研究"
     "确认后方可评估。",
     first_indent=22)
...
para("数据可用性声明：暴露 GWAS（Zhou 等，2020）与结局 GWAS（ebi-a-GCST90018929、"
     ...
     "（805 细胞系 × 198 药物）。全部 19 个分析脚本（脚本 01–17 及 12b、15b，含 MR 敏感性二轮补跑、"
     "MVMR 条件检验与 qhet 置信区间重跑、Lasso/弹性网重跑、Bootstrap 稳定性、"
     "药敏交叉验证与置换检验）"
     "已整理为可复现脚本包，完整代码公开于 GitHub "
     "（https://github.com/qwe2412/TSH-ThyroidCancer-MR，共 11 次提交，含脚本、运行说明、"
