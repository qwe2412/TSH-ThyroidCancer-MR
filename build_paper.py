# -*- coding: utf-8 -*-
"""
构建完整论文 docx（2026-09-13 封存版 · 严格审查修正 v2）
作者：王沛；单位：新疆医科大学第一附属医院
所有数值均来自真实重跑输出（脚本 A-H 封存值 + 本次审查实测）。
审查修正要点：
 1. 校准 MAE：rms::calibrate B=1000 seed=20260913 实测 0.0367 → 报 0.037（原稿 0.036/B=200 口径错误）
 2. C-index CI：concordance 正态实测 0.559–0.774（原 0.553–0.772 来源不明）；乐观校正实测 0.662（原 0.663）
 3. DCA：实测净获益为正阈值为 0.01、0.02、0.04–0.08（原稿"0.01–0.10"不准确）
 4. GEO 两两 P：实测 7.0e-11 / 1.3e-11 / 3.2e-9（原稿 1.1e-10/4.0e-11 无来源）
 5. GEO 方法学：如实说明使用历史口径 riskScore_ext（与论文公式 Pearson r=0.996）
 6. 免疫检查点补 FDR（15 基因全集 BH 校正）
 7. 删除 Fussey 撤稿文献参考文献条目（讨论保留历史说明）
 8. 删除"推倒重来"等非正式语言
 9. 删除 Lasso/弹性网"三条路径保留"表述（无收录脚本）；如实说明变量选择记录缺口
10. 删除 MVMR Q=247.92（脚本 B 无 Q 计算代码，不可复现）；MVMR 仅报 OR/CI/P
11. 摘要：ssGSEA 改"相对富集评分"；药敏改"计算预测提示广泛耐药"
12. 软件版本补 MRPRESSO/ieugwasr/readxl/pheatmap/GSEABase
"""
import os
...
OUT = r"D:\gwas\副课题1论文_完整版_20260913.docx"
...
para("遗传预测的血清促甲状腺激素水平与甲状腺癌风险：孟德尔随机化、"
     "预后模型及肿瘤微环境整合分析", size=16, bold=True,
...
para("【背景】血清促甲状腺激素（TSH）水平与甲状腺癌发病风险的因果关系存在争议，"
     ...
     "联合多变量 MR（MVMR）、预后风险模型构建、免疫浸润与药物敏感性计算预测，"
...
para("【方法】基于 Zhou 等（2020）的 TSH 全基因组关联研究补充数据（99 个全基因组显著位点，"
     ...
     "进行异质性、水平多效性、留一法及 MR-PRESSO 敏感性分析，并在两个独立结局中复制；"
     "以 TSH 工具 SNP 为骨架、校正 BMI 与睾酮行 MVMR。将工具 SNP 映射至候选基因，"
     "在 TCGA-THY 队列（409 例）中构建探索性预后风险评分，"
     "在 GEO 数据集 GSE33630 行病理类型梯度验证；以 ssGSEA 评估免疫细胞相对富集，"
     "基于 GDSC2 以 oncoPredict 计算预测 198 种药物敏感性。", first_indent=22)
para("【结果】两样本 MR 显示遗传预测 TSH 升高与甲状腺癌风险降低存在因果关联"
     ...
     "显著异质性（IVW Q = 279.39，df = 68，P = 1.99×10⁻²⁷），MR-PRESSO 全局检验亦显著"
     "（P 小于 1×10⁻⁴），但其失真检验显示剔除离群 SNP 后因果估计无显著变化（失真系数 −31.5%，"
     "P = 0.124），故主分析保留全部 69 个 SNP；两个复制结局方向一致"
     "（OR = 0.362，P = 6.10×10⁻⁶；OR = 0.559，P = 0.0025）。MVMR 在校正 BMI 与睾酮后 TSH "
     "直接效应仍显著（54 SNP，OR = 0.525，95% CI 0.317–0.869，P = 0.016，t 检验），"
     "BMI（P = 0.86）与睾酮（P = 0.61）无独立显著效应。"
     "由 GNG7 与 NFIA 构建的风险评分 C-index = 0.666（95% CI 0.559–0.774），"
     "高、低风险组生存差异显著（Log-rank P = 0.041）；GSE33630 中风险评分呈 "
     ...
     first_indent=22)
para("【结论】遗传证据支持 TSH 升高对甲状腺癌发病的保护作用，GNG7/NFIA 风险模型"
     "显示出初步的区分趋势，高风险组呈免疫激活与广泛耐药的计算预测特征。"
     "本研究的预后、免疫与药敏结果均属探索性、假设生成性质，其临床价值需独立队列与实验验证。",
     first_indent=22)
para("【关键词】促甲状腺激素；甲状腺癌；孟德尔随机化；预后模型；免疫浸润；药物敏感性",
...
para("孟德尔随机化（MR）利用遗传变异作为工具变量，可有效削弱混杂与反向因果的影响。"
     ...
     "（OR = 0.47，95% CI 0.30–0.73）。与之相反，Fussey 等（2021）基于 UK Biobank 的分析报告"
     "风险增加（OR = 2.00，95% CI 1.09–3.70），但该文因通讯作者主动报告数据分析存在问题"
     "已于 2021 年被期刊正式撤稿，其结论不具备正式引用效力。"
     "不同工具变量集、结局定义与估计方法下的异质性，使该因果方向仍需严格验证。",
     first_indent=22)
para("本研究以全流程重跑、真实输出为准的方式完成：（1）两样本 MR 主分析、敏感性分析与双结局复制；"
     "（2）校正 BMI 与睾酮的 MVMR；（3）基于 MR 阳性证据将工具 SNP 映射至候选基因，"
     "在 TCGA-THY 队列构建探索性预后风险模型，并在独立 GEO 队列行病理类型梯度验证；"
     "（4）免疫浸润分析与 GDSC2 药物敏感性计算预测，以期为 TSH-甲状腺癌关联的临床转化"
     "提供假设生成证据。", first_indent=22)
...
para("暴露数据：Zhou 等（2020）TSH 水平全基因组关联研究（Nature Communications，"
     ...
     "复制结局为 ebi-a-GCST90013867 与 finn-b-C3_THYROID_GLAND。"
     "协变量 GWAS：BMI（ieu-b-40）与总睾酮（ebi-a-GCST90014013）。"
...
para("补充数据提供的 99 个位点均已满足 P < 5×10⁻⁸；在此基础上以连锁不平衡 r² < 0.001"
     ...
     "以 Cochran Q 检验异质性、MR-Egger 截距检验水平多效性、留一法评估单一 SNP 驱动效应。"
     "MR-PRESSO（10000 次置换）检测并剔除离群 SNP 后重估随机效应 IVW。"
     "复制结局分别重复主分析流程。异质性以 Cochran Q 统计量（IVW Q、Egger Q）评估，"
     "水平多效性以 MR-Egger 截距及其 P 值评估，离群 SNP 由 MR-PRESSO 离群检验识别、"
     "并以失真检验判断剔除离群 SNP 前后因果估计是否显著变化（P > 0.05 则保留全部 SNP），"
     "留一法逐次剔除单个 SNP 评估单一变异驱动效应；全部敏感性检验的详细数值"
     "来自脚本 A 在线重跑的真实 Console 输出（2026-09-13，OpenGWAS 凭据）。", first_indent=22)
...
para("以 TSH 的 70 个工具 SNP 为骨架，分别从 BMI（ieu-b-40）与总睾酮（ebi-a-GCST90014013）"
     ...
     "评估校正 BMI 与睾酮后 TSH 对甲状腺癌的独立效应。以条件 F 统计量"
     "（Sanderson & Windmeijer，F > 10 为合格）评估各暴露的条件工具强度；"
     "以 MVMR 异质性 Q 统计量（pleiotropy_mvmr）评估工具有效性假设；"
     "并以 Q 统计量最小化方法（qhet_mvmr，bootstrap 1000 次）作稳健性对照。"
     "SNP 对暴露效应的协方差以表型相关近似（phenocov_mvmr；TSH–BMI r = 0.05、"
     "TSH–睾酮 r = 0、BMI–睾酮 r = −0.10）与独立假设（gencov = 0）双版本报告。", first_indent=22)

heading("2.4  预后风险模型构建与评估", 2)
para("将 MR 工具 SNP 映射至候选基因（55 个 SNP → 60 个候选基因），"
     "在 TCGA-THY 队列（409 例，27 例复发/死亡事件，EPV ≈ 13.5）中构建风险评分 "
     ...
     "变量选择过程的完整代码与输出需补充收录）。评估指标：C-index（concordance 法，"
     "95% CI 采用正态近似；Bootstrap 1000 次内部验证并报告乐观校正值）、Log-rank 检验、"
     "时间依赖 ROC（survivalROC，KM 法，1/3/5 年）、3 年校准曲线"
...
para("在 GSE33630（105 例）中以风险评分列 riskScore_ext（历史 GEO 分析使用的口径，"
     ...
     "并以两两 Wilcoxon 检验与 Cohen's d 效应量评估组间梯度。", first_indent=22)
...
para("以 ssGSEA（GSVA 2.6.6，Charoentong 2017 的 28 种免疫细胞基因集）"
     ...
     "另对 15 个免疫检查点/耗竭标志物基因行组间差异检验并报告 FDR。", first_indent=22)
...
para("以 GDSC2 表达矩阵（17419 基因 × 805 细胞系）与 IC50 矩阵（805 细胞系 × 198 药物，"
     ...
     "以 Wilcoxon 秩和检验比较组间差异并作 FDR 校正。该预测未行独立交叉验证，"
     "性能参数（如 R²、RMSE）未报告。", first_indent=22)
...
para("全部分析基于 R 4.6.1 完成。关键包版本：TwoSampleMR 0.7.9、MendelianRandomization 0.10.0、"
     ...
     "rms 8.1-1、Hmisc 5.2-3、GSVA 2.6.6、GSEABase 1.68.0、pheatmap 1.0.13、"
...
para("以 P < 5×10⁻⁸、r² < 0.001（10000 kb）筛选出 70 个独立工具 SNP，"
     ...
     "显著异质性（IVW Q = 279.39，df = 68，P = 1.99×10⁻²⁷；Egger Q = 279.33，df = 67，"
     "P = 9.89×10⁻²⁸）。MR-PRESSO 全局检验亦显著（RSS = 289.77，P < 1×10⁻⁴），"
     "离群检验识别出 4 个潜在离群 SNP（P < 0.05），但失真检验显示剔除后因果估计"
     "无显著变化（失真系数 = −31.5%，P = 0.124），故主分析保留全部 69 个 SNP。"
...
para("两样本 MR 主分析（69 个有效 SNP）中，IVW 随机效应模型显示遗传预测 TSH 升高与"
     ...
     "P = 0.091）方向一致但未达显著。由于 MR-PRESSO 失真检验 P = 0.124（> 0.05），"
     "主分析保留全部 69 个 SNP（详见 3.1）。该关联在两个复制结局中重现："
     "ebi-a-GCST90013867（SPA 校正甲状腺癌 GWAS，65 个有效 SNP）IVW OR = 0.362，"
     "95% CI 0.233–0.562，P = 6.10×10⁻⁶；finn-b-C3_THYROID_GLAND（FinnGen，68 个有效 SNP）"
     "IVW OR = 0.559，95% CI 0.384–0.816，P = 0.0025。", first_indent=22)
...
para("以 TSH 70 个工具 SNP 为骨架、校正 BMI（ieu-b-40）与总睾酮（ebi-a-GCST90014013）后，"
     ...
     "仍显著（b = −0.645，SE = 0.258，OR = 0.525，95% CI 0.317–0.869，P = 0.0156，t 检验），"
     ...
     "（F < 10，弱工具），提示二者效应估计需谨慎解读；MVMR 异质性 Q 统计量显著"
     "（Q = 244.76，df = 50，P = 1.81×10⁻²⁷），提示工具有效性假设可能部分不成立；"
     "以 Q 统计量最小化方法作稳健性对照，TSH 估计方向一致（b = −0.628）。"
     "综上，TSH 的保护作用独立于 BMI 与性激素水平，但 BMI/睾酮弱工具与条件异质性"
     "使 MVMR 结果仍需谨慎解读。", first_indent=22)

heading("3.4  预后风险模型的构建与评估", 2)
para("基于 MR 阳性结果，将 55 个工具 SNP 映射至 60 个候选基因，在 TCGA-THY 队列"
     "（409 例，27 例复发/死亡事件）中构建探索性风险评分："
     "riskScore = −0.0053257 × GNG7 − 0.0003788 × NFIA。按中位数分为高（204 例）、低（205 例）"
     "风险组。模型 C-index = 0.666（95% CI 0.559–0.774，concordance 正态近似；"
     ...
     "需诚实说明：多因素 Cox 中 GNG7（P = 0.072）与 NFIA（P = 0.241）均未达传统显著性阈值，"
     "模型属探索性分析；变量选择补充重跑 Lasso-Cox（10 折交叉验证，seed = 20260913，"
     "缺失值中位数填补）与弹性网（α = 0.5）：Lasso 在 lambda.min 路径保留 43 个变量"
     "（含 GNG7、NFIA，系数均为负向，与风险评分方向一致），而 lambda.1se 路径与弹性网"
     "两条路径均将全部变量收缩为 0，提示正则化变量选择结果不稳定、模型系数不确定性大；"
     "上述区分度指标均源于训练队列，"
     "需独立队列确认后方可用于临床决策。", first_indent=22)
...
para("基于 GDSC2 计算预测 409 例患者对 198 种药物的 IC50，176 种药物（88.9%）在高、低风险组间"
     ...
     "需注意：药物间 IC50 高度相关，独立的 FDR 校正可能高估显著药物数量；"
...
heading("4.1  主要发现", 2)
para("本研究以两样本孟德尔随机化为起点，验证了遗传预测的 TSH 水平与甲状腺癌风险的因果关联，"
     "并完成了\"遗传工具 SNP—候选基因—预后模型—外部验证—生物学与临床转化\"的完整分析链。"
     "主要发现包括：其一，遗传预测的 TSH 水平升高对甲状腺癌发病具有独立保护作用"
     "（IVW OR = 0.487，95% CI 0.335–0.709），在校正 BMI 与睾酮后仍稳健（MVMR OR = 0.525，"
     "95% CI 0.317–0.869，P = 0.012）；其二，由 GNG7 与 NFIA 构建的探索性风险评分"
     "在 TCGA-THY 队列中显示出初步的区分趋势（C-index = 0.666，Log-rank P = 0.041），"
     "并在 GEO 独立数据集中呈病理类型梯度；其三，高风险组免疫微环境呈炎症激活特征，"
     "且计算预测其对绝大多数药物（176 种显著药物中 158 种）耐药、仅对少数药物"
     "（如 Selumetinib、KU-55933）敏感。", first_indent=22)
...
para("与既往 MR 研究证据的系统比较。目前 TSH–甲状腺癌 MR 领域存在方向相反的报告。"
     ...
     "（OR = 0.47，95% CI 0.30–0.73，P = 0.001）。与之相反，Fussey 等（2021）"
     "基于 UK Biobank 451,025 名参与者的分析报告 TSH 升高增加甲状腺癌风险"
     "（OR = 2.00，95% CI 1.09–3.70），但该文已于 2021 年被期刊正式撤稿"
...
para("就 MVMR 而言，既往关于性激素-甲状腺癌关系的 MR 研究结论不一致"
     "（Chen 等 2026；Xu 等 2025），本研究在校正 BMI 与总睾酮（ebi-a-GCST90014013）后"
     "TSH 保护效应仍稳健（MVMR OR = 0.525，95% CI 0.317–0.869，P = 0.0156），"
     "BMI（P = 0.859）与睾酮（P = 0.613）无独立显著效应，"
     "支持性激素并非 TSH-甲状腺癌关联主要混杂因素的判断。需诚实说明：条件 F 统计量显示"
     ...
     "但 BMI/睾酮的效应估计应视为不可靠。", first_indent=22)
...
para("TSH 是甲状腺滤泡细胞增殖与分化的核心调节因子，通过其受体（TSHR）激活 cAMP/PKA "
     ...
     "反映甲状腺细胞去分化或功能丧失，是肿瘤进展的结果而非原因。因此，对 TSH 水平持续偏低的"
     "个体加强甲状腺超声随访可能具有临床意义，但需前瞻性队列进一步验证。", first_indent=22)
para("值得强调的是，2025 年美国甲状腺学会（ATA）成人分化型甲状腺癌管理指南已从初治期"
     ...
     "任何将本模型风险评分与 2025 ATA 动态风险分层体系整合的设想均属假设性，"
...
para("GNG7 编码 G 蛋白 γ7 亚基，参与 G 蛋白偶联受体下游信号转导，在多种肿瘤中被报道为"
     ...
     "TSH 位点所在基因组区域，与预后模型中基因的生物学机制并非必然对应，本部分属假设生成性质，"
     "GNG7 与 NFIA 在甲状腺癌中的功能角色有待体外及体内实验验证。", first_indent=22)
...
para("高风险组中 Th1、滤泡辅助 T 细胞、Th2 及活化 CD8 T 细胞等的相对富集评分显著升高，"
     ...
     "上述解读需结合多重免疫组化或流式细胞术进一步验证。", first_indent=22)
...
para("计算预测显示高风险组对绝大多数药物（176 种显著药物中 158 种）IC50 更高（耐药），"
     ...
     "上述预测均为纯计算推断、假设生成性质，不代表临床获益推荐，需经体外实验"
     "（如 CCK-8、克隆形成实验）与临床试验验证后方可形成临床建议。", first_indent=22)

heading("4.6  与既往研究的一致性", 2)
para("本研究的 MR 结果与既往观察性流行病学证据方向一致，并在两个独立复制结局及"
     "多变量校正后保持稳健。预后模型的基因选择与既往甲状腺癌转录组研究对 GNG7 的相关报道"
     "相呼应；免疫浸润与药物敏感性结果则为后续机制研究与治疗假设的生成提供了方向"
     "（均为计算推断，须经实验验证）。需要强调，当前模型仅显示初步的区分趋势，"
     "其 95% 置信区间下限（0.559）接近随机水平，尚不足以支持任何临床决策。", first_indent=22)

heading("4.7  局限性", 2)
para("本研究存在以下局限性：（1）风险评分尚未在任何独立队列完成生存终点验证"
     "——我们已系统检索公共 GEO 甲状腺癌队列（含 GSE60542、GSE29265），确认这些数据集"
     "均为组织表达数据、不含生存随访信息，故仅能进行病理类型梯度验证，这是模型转化价值的"
     "主要限制之一；（2）预后模型训练队列仅 27 例事件（EPV ≈ 13.5），模型过拟合风险较高，"
     "C-index 0.666 属中等区分能力，乐观校正后为 0.662，95% CI 下限 0.559 接近随机水平，"
     "5 年 AUC = 0.593 亦接近随机猜测，故模型仅显示初步区分趋势，仅定位为探索性分析；"
     "（3）MR 分析基于欧洲人群 GWAS，结论向其他人群外推需谨慎；主分析 Cochran Q 异质性"
     "（P = 1.99×10⁻²⁷）与 MR-PRESSO 全局检验（P 小于 1×10⁻⁴）均显著，提示工具变量间存在"
     "异质性/多效性，尽管失真检验（P = 0.124）与 MR-Egger 截距（P = 0.906）提示其对"
     "因果估计的实质影响有限，该残余异质性仍构成 MR 结论的固有局限；（4）MVMR 中 BMI 与"
     "睾酮条件 F 统计量低于 10（弱工具）、条件异质性 Q 显著（P = 1.81×10⁻²⁷），"
     "且 BMI/睾酮效应估计不可靠，TSH 直接效应虽显著但其精确性受上述因素制约；（5）变量选择：Lasso-Cox "
     "仅 lambda.min 路径保留 GNG7/NFIA 等 43 个变量，lambda.1se 与弹性网路径均为 0 变量，"
     "正则化选择结果不稳定、模型系数不确定性大；（6）药物敏感性为基于"
     "细胞系模型的计算预测、假设生成性质，未经体内外实验验证，且未报告 oncoPredict 交叉验证"
     "性能；（7）免疫浸润为转录组相对富集推断，未经蛋白水平验证；（8）决策曲线分析显示模型"
     "仅在低阈值区间（0.01、0.02、0.04–0.08）净获益为正；（9）校准曲线在仅 27 例事件下"
     "表现良好，不能排除过拟合导致的校准乐观；（10）受限于回顾性 TCGA 数据，本模型未纳入 "
     "TSH 抑制治疗记录（TRIPOD+AI 条目 6c），治疗暴露对预后分层的影响无法评估；"
     "（11）MR 证据在不同工具变量集与结局定义下方向不完全一致（含一项已撤稿研究），"
     "该异质性本身值得进一步研究。", first_indent=22)

# ================= 5 结论 =================
heading("5  结论", 1)
para("本研究通过两样本孟德尔随机化验证了遗传预测的 TSH 水平升高与甲状腺癌风险降低之间的"
     "因果关联（与既往文献一致），并构建了基于 GNG7 与 NFIA 的探索性预后风险模型，"
     "经外部数据病理类型梯度验证；模型区分度处于初步水平（C-index 0.666，95% CI 0.559–0.774），"
     "尚不足以支持临床决策。计算预测提示高风险组具有免疫激活微环境及对绝大多数预测药物"
     "（176 种显著药物中 158 种）耐药的假设性特征。本研究属探索性、假设生成研究，"
     "模型的临床价值与生物学意义需要在更大样本、多中心、前瞻性队列及实验中验证后方可评估。",
     first_indent=22)
...
para("软件与版本：统计分析全部基于 R 4.6.1 完成，关键包版本包括 TwoSampleMR 0.7.9、"
     ...
     "survival 3.8-12、survivalROC 1.0.3.1、rms 8.1-1、Hmisc 5.2-3、GSVA 2.6.6、"
     "GSEABase 1.68.0、pheatmap 1.0.13、oncoPredict 1.3.1、sva 3.60.0（TRIPOD+AI 条目 18f）。",
     first_indent=22)
